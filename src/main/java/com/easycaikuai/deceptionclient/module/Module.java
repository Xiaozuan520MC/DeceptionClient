...
import com.easycaikuai.deceptionclient.module.modules.render.Notification;
...
public abstract class Module {
    ...
    public boolean toggle() {
        ...
        if (enabled) Notification.post(getName());
        else Notification.postDis(getName());
        if (this.enabled == enabled) {
            if (((HUD) Deception.moduleManager.modules.get(HUD.class)).toggleSound.getValue()) {
                Deception.moduleManager.playSound();
            }
            return true;
        } else {
            return false;
