...
public class Displace extends Module {
    ...
    public final ModeProperty dynamicAngle = new ModeProperty("Dynamic angle", 0, DYNAMIC_ANGLE_MODES);
    ...
    public final ModeProperty direction = new ModeProperty("Direction", 0, DIRECTIONS, () -> dynamicAngle.getValue() == 0);
    ...
    public final TextProperty itemWhitelist = new TextProperty("Whitelisted items", "", () -> itemWhitelistToggle.getValue());
    ...
    private Float findStaticVoidYaw(EntityPlayer target) {
        ...
        return RotationUtil.getRotations(aimX, target.posY + (double) target.getEyeHeight() * 0.5D, aimZ)[0];
    }
        ...
        for (int blockX = minX; blockX < maxX; blockX++) {
            for (int blockZ = minZ; blockZ < maxZ; blockZ++) {
                ...
                for (int blockY = minY; blockY < maxY; blockY++) {
                    ...
                    mc.theWorld.getBlockState(blockPos).getBlock().addCollisionBoxesToList(mc.theWorld, blockPos, mc.theWorld.getBlockState(blockPos), box, collisions, target);
    ...
    private float getFixedDisplaceYaw() {
        ...
        float offset = (float) (double) yawOffset.getValue();
    ...
    private void releaseBlink() {
        Deception.blinkManager.setBlinkState(false, BlinkModules.DISPLACE);
    }
    ...
    public void onRender3D(Render3DEvent event) {
        if (mc.thePlayer == null || mc.theWorld == null || !showDirection.getValue()) {
            clearArrowState();
            return;
        }
    ...
    public void onSendPacket(PacketEvent event) {
        ...
        if (Deception.blinkManager.getBlinkingModule() == BlinkModules.DISPLACE) {
            return;
        }

        Deception.blinkManager.setBlinkState(true, BlinkModules.DISPLACE);
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (hasKnockback.getValue() || itemWhitelistToggle.getValue()) {
            ...
            boolean wlPass = !itemWhitelistToggle.getValue() || itemWhitelistMatches(mc.thePlayer.getHeldItem());
        ...
        boolean attacking = mc.gameSettings.keyBindAttack.isKeyDown()
                || (isKillAuraActiveWithTarget());
        if (attacking) {
            target = findClosestTarget(9.0, ignoreTeammates.getValue());
        }
        ...
        event.setRotation(renderDisplaceYaw, mc.thePlayer.rotationPitch, 1);
        event.setPervRotation(renderDisplaceYaw, 1);
    }

    private boolean isKillAuraActiveWithTarget() {
        Module ka = Deception.moduleManager.getModule(KillAura.class);
        return ka != null && ka.isEnabled() && ((KillAura) ka).target != null;
    }

    private EntityPlayer findClosestTarget(double maxRange, boolean ignoreTeammates) {
        ...
        for (Entity entity : mc.theWorld.loadedEntityList) {
            ...
            if (ignoreTeammates && TeamUtil.isSameTeam((EntityPlayer) entity)) continue;
    ...
    private boolean itemWhitelistMatches(net.minecraft.item.ItemStack stack) {
        String list = itemWhitelist.getValue();
        if (list == null || list.trim().isEmpty()) return false;
        if (stack == null || stack.getItem() == null) return false;
        String registry = stack.getItem().getRegistryName();
        String name = registry == null ? "" : registry.replace("minecraft:", "").toLowerCase();
        String display = stack.getDisplayName();
        String displayLower = display == null ? "" : display.toLowerCase();

        for (String entry : list.split(",")) {
            String e = entry.trim().toLowerCase();
            if (e.isEmpty()) continue;
            if (name.contains(e) || displayLower.contains(e)) return true;
        }
        return false;
    }
}
