...
public class Radar extends Module {
    ...
    private Color getEntityColor(EntityPlayer entityPlayer) {
        if (TeamUtil.isFriend(entityPlayer)) {
            Color color = Deception.friendManager.getColor();
            return new Color(color.getRed(), color.getGreen(), color.getBlue(), 255);
        } else if (TeamUtil.isTarget(entityPlayer)) {
            Color color = Deception.targetManager.getColor();
            return new Color(color.getRed(), color.getGreen(), color.getBlue(), 255);
        } else {
            switch (this.colorMode.getValue()) {
                ...
                case 2:
                    int color = ((HUD) Deception.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis()).getRGB();
    ...
    public void onRender(Render2DEvent event) {
        ...
        for (EntityPlayer player : TeamUtil.getLoadedEntitiesSorted().stream().filter(entity -> entity instanceof EntityPlayer && this.shouldRender((EntityPlayer) entity)).map(EntityPlayer.class::cast).collect(Collectors.toList())) {
            ...
            com.easycaikuai.deceptionclient.util.shader.RoundedUtils.drawRound((float)px - dotRadius.getValue(), (float)py - dotRadius.getValue(), dotRadius.getValue() * 2, dotRadius.getValue() * 2, dotRadius.getValue(), new Color(getEntityColor(player).getRGB(), true));
