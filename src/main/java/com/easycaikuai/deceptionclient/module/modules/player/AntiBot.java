...
public class AntiBot extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        for (EntityPlayer bot : bots) {
            ...
            if (debug.getValue()) {
                ChatUtil.sendFormatted(String.format("%sAntiBot: &fRemoved &r%s &fdue to it being a bot.", Deception.clientName, bot.getName()));
            }
        }
    }
    ...
    private void handleMatrixBot() {
        ...
        if (matrixCollectSample) {
            ...
            for (EntityPlayer bot : bots) {
                ...
                if (debug.getValue()) {
                    ChatUtil.sendFormatted(String.format("%sAntiBot/MatrixBot: &fRemoved &r%s&f. Speed: &b%.2f&f.", Deception.clientName, bot.getName(), getHorizontalSpeed(bot)));
                }
            }
        }
    ...
    private void handleSpawnPlayerPacket(S0CPacketSpawnPlayer packet) {
        Module killAura = Deception.moduleManager.getModule("KillAura");
        if (killAura != null && killAura != null && killAura.isEnabled() && !hasRemovedEntities.contains(packet.getEntityID())) {
    ...
    public static boolean isBot(EntityLivingBase entity) {
        AntiBot antiBot = (AntiBot) Deception.moduleManager.getModule(AntiBot.class);
