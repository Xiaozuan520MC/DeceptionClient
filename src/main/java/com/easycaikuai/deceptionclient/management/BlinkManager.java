...
public class BlinkManager {
    ...
    public boolean offerPacket(Packet<?> packet) {
        if (this.blinkModule == BlinkModules.NONE || packet instanceof C00PacketKeepAlive || packet instanceof C01PacketChatMessage) {
            ...
            return false;
        } else if (this.blinkModule == BlinkModules.AUTO_BLOCK && Deception.moduleManager != null) {
            KillAura killAura = (KillAura) Deception.moduleManager.modules.get(KillAura.class);
            if (killAura != null && killAura.noBlinkAttackPackets.getValue() && packet instanceof net.minecraft.network.play.client.C02PacketUseEntity) {
                return false;
            }
